...
export default function RecordedScreen() {
  ...
  const recordedContext: PlaybackContext = useMemo(
    () => ({
      type: 'all',
      title: 'Recorded VNs',
      items: filteredVns,
  ...
  function handlePlayAll() {
    if (filteredVns.length > 0) {
      playVn(filteredVns[0], 0, recordedContext);
    }
  }
  ...
  function handleShufflePlay() {
    if (filteredVns.length > 0) {
      ...
      playVn(filteredVns[randomIndex], 0, recordedContext);
    }
  }

  function handlePlayTrack(track: VN) {
    playVn(track, 0, recordedContext);
  }
